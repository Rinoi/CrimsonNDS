...
TBackground::TBackground(const Type &type, BgSize size, SpriteSheet *spriteSheet, int depth) :
...
TBackground::TBackground(const Type &type, BgSize size, SpriteSheet *spriteSheet, u16 *map, u_int width, u_int height, int depth) :
...
u16		TBackground::getIdx16(u16 idx)
{
  u16		r = 0;
...
void		TBackground::addInMapT16(int x, int y, u16 idx)
{
  int		bMap = 0;
...
void		TBackground::addInMapT16(u16 *map, u_int width, u_int height, int bMap)
{
...
void		TBackground::initT16(u16 idx)
{
...
void		TBackground::initT16(u16 idx, int bMap)
{
...
void		TBackground::addInMapT32(int x, int y, u16 idx)
{  
  int		i = y * 32 * 4 + x * 4;
...
void		TBackground::addInMapT32(u16 *map, u_int width, u_int height)
{
  for (u_int y = 0; y < height; y += 1)
    for (u_int x = 0; x < width; x += 1)
...
void		TBackground::initT32(u16 idx)
{
...
u16		*TBackground::getGfx()
{
  return (bgGetGfxPtr(this->id));
}

u16		*TBackground::getMap()
{
  return (this->map);
}

void		TBackground::scroll(int dx, int dy)
{
  bgSetScroll(this->id, dx, dy);
}

void		TBackground::init(BgSize size)
{
