...
bool		engineInit()
{
  bg = new TBackground(TBackground::SUB, BgSize_T_512x256, &rogueLikeSheet, TileLayer1, 16, 16);

  bMap = new BMap({16, 16}, {2, 1}, true);
  CMap = new BColliderMap(bMap, {16, 16}, {33});
  bMap->init(1);
  bMap->setMap(0, mapA);
  bMap->setMap(1, mapA);
  ...
  // bg->addInMapT16(bMap->getData(), 16, 16, 0);
